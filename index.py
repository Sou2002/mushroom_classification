...
with st.form("form 1"):
    ...
    my_dict['cap_shape'] = col1.selectbox('Cap Shape', options=(
        'Bell', 'Conical', 'Convex', 'Flat', 'Knobbed'), index=None, placeholder='Choose an option')
    my_dict['cap_surface'] = col2.selectbox('Cap Surface', options=(
        'Fibrous', 'Grooves', 'Scaly', 'Smooth'), index=None, placeholder='Choose an option')
    my_dict['cap_color'] = col3.selectbox('Cap Color', options=('Brown', 'Buff', 'Cinnamon',
    ...
    my_dict['gill_attachment'] = col3.selectbox('Gill Attachment', options=(
        'Attached', 'Descending', 'Free', 'Notched'), index=None, placeholder='Choose an option')
    my_dict['gill_apacing'] = col1.selectbox('Gill Spacing', options=('Closed', 'Crowded',
                   'Distant'), index=None, placeholder='Choose an option')
    my_dict['gill_size'] = col2.selectbox('Gill Size', options=('Broad', 'Narrow'),
                   index=None, placeholder='Choose an option')
    my_dict['gill_color'] = col3.selectbox('Gill Color', options=('Black', 'Brown', 'Buff', 'Chocolate',
                                          'Gray', 'Green', 'Orange', 'Pink', 'Purple', 'Red', 'White', 'Yellow'), index=None, placeholder='Choose an option')
    my_dict['stalk_shape'] = col1.selectbox('Stalk Shape', options=('Enlarging', 'Tapering'),
                   index=None, placeholder='Choose an option')
    my_dict['stalk_root'] = col2.selectbox('Stalk Root', options=(
        'Bulbous', 'Club', 'Cup', 'Equal', 'Rhizomorphs'), index=None, placeholder='Choose an option')
    my_dict['stalk_surface_above_ring'] = col3.selectbox('Stalk Serface Above Ring', options=(
        'Fibrous', 'Scaly', 'Silky', 'Smooth'), index=None, placeholder='Choose an option')
    my_dict['stalk_surface_below_ring'] = col1.selectbox('Stalk Serface Below Ring', options=(
        'Fibrous', 'Scaly', 'Silky', 'Smooth'), index=None, placeholder='Choose an option')
    my_dict['Stalk_color_above_ring'] = col2.selectbox('Stalk Color Above Ring', options=(
        'Brown', 'Buff', 'Cinnamon', 'Gray', 'Orange', 'Pink', 'Red', 'White', 'Yellow'), index=None, placeholder='Choose an option')
    my_dict['stalk_color_below_ring'] = col3.selectbox('Stalk Color Below Ring', options=(
        'Brown', 'Buff', 'Cinnamon', 'Gray', 'Orange', 'Pink', 'Red', 'White', 'Yellow'), index=None, placeholder='Choose an option')
    my_dict['veil_type'] = col1.selectbox('Veil Type', options=('Partial', 'Universal'),
                   index=None, placeholder='Choose an option')
    my_dict['veil_color'] = col2.selectbox('Veil Color', options=(
        'Brown', 'Orange', 'White', 'Yellow'), index=None, placeholder='Choose an option')
    my_dict['ring_number'] = col3.selectbox('Ring Number', options=('None', 'One', 'Two'),
                   index=None, placeholder='Choose an option')
    my_dict['ring_type'] = col1.selectbox('Ring Type', options=('Cobwebby', 'Evanescent',
                   'Flaring', 'Large', 'None', 'Pendant', 'Sheathing', 'Zone'), index=None, placeholder='Choose an option')
    my_dict['spore_print_color'] = col2.selectbox('Spore Print Color', options=('Black', 'Brown', 'Buff',
